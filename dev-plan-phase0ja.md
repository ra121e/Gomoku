...
現状のリポジトリは `app / PostgreSQL` を中心にした Docker 構成と、Next.js + Socket.IO + Prisma の疎通確認まで完了している。ゲーム盤面・ルール判定・マッチング・認証・履歴・ランキングはまだ未実装。

また、実装ディレクトリは `apps/frontend` / `apps/backend` 分離ではなく、`app/` への一本化方針に移行している。
...
`playerId` は、マッチ作成または参加の成功時にバックエンドが返す `MatchParticipant.id` を指す。フロントエンドはこれを **`sessionStorage`** に保持する。
...
| ③ フェッチ → レンダリング          | `GET /matches` と `GET /matches/:id/state` で一覧・状態取得 |
| ④ サーバープッシュ                 | `game:update` を match 全員へ配信し、盤面を同期             |
...
```text
プレイヤー1:
  名前入力 → POST /matches でマッチ作成
          → Match を 1 件作成
          → MatchParticipant(role=PLAYER, seat=BLACK) を 1 件作成

プレイヤー2:
  名前入力 → GET /matches で一覧取得
           → POST /matches/:id/join で参加
...
着手時:
  ...
  7. 保存成功後、game:update を match 全員へ配信する
...
Player1       Frontend1      Backend           DB           Frontend2
  ...
  |               | <-----------| matchId/player|                |
  |               |             |               |                |
  |      (Player2 が GET /matches → POST /join で参加)           |
  |               |             | INSERT participant(WHITE)      |
  |               |             | UPDATE Match.status=IN_PROGRESS|
  |               |             |               |                |
  |      (両者が WebSocket で match を購読)                      |
  |               | WS subscribe|               |                |
  |               |------------>|               |                |
  |               |             |<--------------| WS subscribe   |
  |               |             |               |                |
...
app/
  ...
  lib/
    ...
    socket/
      game-handler.ts         ← match:subscribe と game:update 配信
prisma/
  schema.prisma
```
